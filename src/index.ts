...
export class Cache<K, V> extends Map<K, v<V>> {
    ...
    constructor(duration?: number) {
        super();
        if (duration) {
            if (typeof duration !== 'number') throw new TypeError('Argument time must be a number');
    ...
    public valid(key: K): boolean {
        ...
        if (!val) return false;
        return Math.abs((val.createdAt.getTime() - new Date().getTime()) / 1000) < this.duration;
    }
    /**
    ...
    public add(key: K, value: V): V | undefined {
        if (!key || !value) throw new TypeError('Arguments keys and values are required');
        this.set(key, {
            data: value,
            createdAt: new Date()
        })
